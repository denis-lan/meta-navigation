...
from objectmaze_sb_intcode_exposures_fixed2landmarks import ObjectMaze as ObjectMazeSB
...
def main():
    ...
    for model_num in model_nums:
        #choose random seed between 1 and 10000
        env_seed = np.random.randint(1, 10000)
        #choose random seed between 10001 and 20000
        # env_seed = np.random.randint(10001, 20000)
        ...
        np.random.seed()
        # num_landmarks =  np.random.choice([2, 4, 8, 16])
        num_landmarks = 2
        num_landmarks_trials = [num_landmarks] * 1000
        action_conditions = np.repeat([0], len(num_landmarks_trials))
        obstacles_trials = np.repeat([0], len(num_landmarks_trials) * 2) 
        ...
        for i in range(num_trials):
            ...
            obs, info = env.reset(num_landmarks = num_landmarks, obstacles = obstacles, actioncondition = action_condition, seed = env_seed)
            ...
            while not done and not truncated:
                ...
                action, lstm_states = model.predict(obs, state=lstm_states, episode_start=episode_start, deterministic=False, action_masks = action_masks)
                handle.remove()
        ...
        df.to_hdf('10 models v4/envseeds/model' + str(model_num) + '_2landmarksv1.hdf', key = 'df', mode = 'w')
