...
function calculateDistanceToGoal(goalxloc, goalyloc){
  distanceToGoal = new Array(coding.numrows);
  for (i = 0; i < coding.numrows; i++){
    distanceToGoal[i] = Array(coding.numcols).fill(-1);
  }
  distanceToGoal[goalxloc][goalyloc] = 0;

  for(i = 0; i < parameters.pathlength; i++){
    for(x = 0; x < coding.numcols; x++){
      for(y = 0; y < coding.numrows; y++){
        if(distanceToGoal[x][y] == i){
          
          if(x-1 >= 0){
            if(goalorstart[x-1][y] != 9998 && distanceToGoal[x-1][y] == -1){
              distanceToGoal[x-1][y] = i + 1;
            }
          }

          if(x+1 < coding.numcols){
            if(goalorstart[x+1][y] != 9998 && distanceToGoal[x+1][y] == -1){
              distanceToGoal[x+1][y] = i + 1;
            }
          }
          ...
          if(y-1 >= 0){
            if(goalorstart[x][y-1] !=  9998 && distanceToGoal[x][y-1] == -1){
              distanceToGoal[x][y-1] = i + 1;
            }
          }

          if(y+1 < coding.numrows){
            if(goalorstart[x][y+1] != 9998 && distanceToGoal[x][y+1] == -1){
              distanceToGoal[x][y+1] = i + 1;
            }
          }
...
function generateLandmarks(){
  ...
  //generate obstacles
  //such that there is one obstacle in each row and each column
  sdata.obstaclexlocs[coding.index] = new Array();
  sdata.obstacleylocs[coding.index] = new Array();
  rows = [];
  cols = [];
  for(i = 1; i < coding.numrows - 1; i++){
    rows.push(i);
    cols.push(i);
  }
  rows = shuffle(rows);
  cols = shuffle(cols);
  // console.log(board.objectimg[coding.block][coding.trial]);
  for(i = 0; i < coding.numrows - 2; i++){
    goalorstart[rows[i]][cols[i]] = 9998; // set as obstacle
    // console.log(rows[i]);
    // console.log(cols[i]);
    sdata.obstaclexlocs[coding.index].push(rows[i]);
    sdata.obstacleylocs[coding.index].push(cols[i]);
    board.objectimg[coding.block][coding.trial][rows[i]][cols[i]] = 9998;
    // console.log(board.objectimg[coding.block][coding.trial]);
  }
  sdata.cond_obstaclexlocs[coding.index] = sdata.obstaclexlocs[coding.index].toString();
  sdata.cond_obstacleylocs[coding.index] = sdata.obstacleylocs[coding.index].toString();
  //generate landmarks
  trycount = 0;
...
function sanityCheck(){
  //check if it is always possible to generate a goal when there are 16 landmarks
  sdata.cond_numlandmarks[coding.index] = 16;
  for (var tryno = 0; tryno < 1000; tryno++){
    generateLandmarks();
    generateExploreForcedClicks();
    console.log('success ' + tryno);
  }
}
...
function generateGoalAndStart(){
  ...
  //         if (goalorstart[i][j] == 9998 || sdata.resp_explorexclicks[coding.index][k] == i.toString() && sdata.resp_exploreyclicks[coding.index][k] == j.toString()){
  ...
  //   distanceToGoal = calculateDistanceToGoal(eligiblegoals[chosengoal][0], eligiblegoals[chosengoal][1]);
  ...
  //       if (goalorstart[i][j] == 9998 || distanceToGoal[i][j] != parameters.pathlength){
  //         eligible = false;
  //       }
  //       else{
  //         for(k = 0; k < sdata.resp_explorexclicks[coding.index].length; k++){
  //           if (sdata.resp_explorexclicks[coding.index][k] == i.toString() && sdata.resp_explorexclicks[coding.index][k] == j.toString()){
  //             console.log('not eligible')
  //             eligible = false;
  //             break
  //           }
  ...
  distanceToGoal = calculateDistanceToGoal(candgoalxloc, candgoalyloc);
  // console.log(distanceToGoal);
  xloc = Math.floor(Math.random() * (coding.numrows-4)) + 2;
  yloc = Math.floor(Math.random() * (coding.numrows-4)) + 2;
  trycount = 0;
  while((goalorstart[xloc][yloc]!=0 || distanceToGoal[xloc][yloc] != parameters.pathlength)){
    if (trycount > 100){
      console.log('too many tries');
      generateGoalAndStart();
      break;
    ...
    yloc = Math.floor(Math.random() * (coding.numrows-4)) + 2;
    trycount++;
    // console.log(distanceToGoal[xloc][yloc]);
  }
...
function updateArrows(){
  if (coding.stepactioncondition == 'tonly'){
    board.leftclick.rectangle.object.attr({"stroke": "grey"});
    board.rightclick.rectangle.object.attr({"stroke": "grey"});
    board.upclick.rectangle.object.attr({"stroke": "grey"});
    board.downclick.rectangle.object.attr({"stroke": "grey"});
  }
  else{
    board.leftclick.rectangle.object.attr({"stroke": "black"});
    board.rightclick.rectangle.object.attr({"stroke": "black"});
    board.upclick.rectangle.object.attr({"stroke": "black"});
    board.downclick.rectangle.object.attr({"stroke": "black"});
  }
