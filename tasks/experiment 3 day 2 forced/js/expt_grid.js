function randomiseObjects(){
  ...
  randomList = Array(totObjs).fill(0).map((e,i)=>i+1).filter(e => e % 2 == 0).sort(() => Math.random() - 0.5); //array of 1 to totObjs in random order
...
function generateLandmarks(){
  ...
  if(coding.block == 0){
    ...
    // choose uniformly between 2 and 16
    numlandmarks = Math.floor(Math.random() * (17-2)) + 2;
    // numlandmarks = 8;
    sdata.cond_numlandmarks[coding.index] = numlandmarks;
  }
  for(i = 0; i < numlandmarks; i++){
    candxloc = Math.floor(Math.random() * (coding.numrows-2))+1;
    candyloc = Math.floor(Math.random() * (coding.numrows-2))+1;
    while(goalorstart[candxloc][candyloc] !=0){
      trycount++;
      candxloc = Math.floor(Math.random() * (coding.numrows-2))+1;
      candyloc = Math.floor(Math.random() * (coding.numrows-2))+1;
    ...
    sdata.blocklandmarkxlocs[coding.block][coding.trial][i] = candxloc;
    sdata.blocklandmarkylocs[coding.block][coding.trial][i] = candyloc;
    oldcandxloc = candxloc;
    oldcandyloc = candyloc;
    console.log(candxloc);
    goalorstart[candxloc][candyloc] = 3;
  }
}
...
function generateGoalAndStart(){
  ...
  // if(True){
    ...
    trycount = 0;
    while((goalorstart[xloc][xloc]==3|| abs(coding.goalxloc-xloc) + abs(coding.goalyloc - yloc) != parameters.pathlength)){
    // goalorstart[xloc][xloc] replicates the bug in the free condition where the start can be placed at a landmark location
      if (trycount > 100){
        console.log('too many tries');
        generateGoalAndStart();
        break;
