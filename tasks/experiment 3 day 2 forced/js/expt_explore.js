  ...
  function generateExploreForcedClicks(){
    ...
    if (coding.block == 0){
      ...
      for (i = 0; i < numclicks; i++){
        sdata.exploreforcedxlocs[coding.index][i] = sdata.blocklandmarkxlocs[coding.block][coding.trial][i];
        sdata.exploreforcedylocs[coding.index][i] = sdata.blocklandmarkylocs[coding.block][coding.trial][i];
      }
      ...
      numlandmarks = sdata.cond_numlandmarks[coding.index];
      // list of nan 
      inds = new Array(numclicks).fill(NaN);

      if (numlandmarks == 2){
        // just alternate between the two landmarks
        for (i = 0; i < numclicks; i++){
          inds[i] = i % 2;
        }
        sdata.exploreforcedxlocs[coding.index] = new Array(numclicks);
        sdata.exploreforcedylocs[coding.index] = new Array(numclicks);
        isGoal = new Array(numclicks);
        for (i = 0; i < numclicks; i++){
          sdata.exploreforcedxlocs[coding.index][i] = sdata.blocklandmarkxlocs[coding.block][coding.trial][inds[i]];
          sdata.exploreforcedylocs[coding.index][i] = sdata.blocklandmarkylocs[coding.block][coding.trial][inds[i]];
          isGoal[i] = false;
        }
      }
      else{
        //first allocate the landmarks into random positions
        for (i = 0; i < numlandmarks; i++){
          //choose a random ind
          ind = Math.floor(Math.random() * numclicks);
          //if the ind is already chosen, choose another one
          while(!isNaN(inds[ind])){
            ind = Math.floor(Math.random() * numclicks);
          }
          inds[ind] = i;
        }

        //then, for all remaining nans, allocate a random landmark
        for (i = 0; i < numclicks; i++){
          if(isNaN(inds[i])){
            ind = Math.floor(Math.random() * numlandmarks);
            while((i > 0 && inds[i-1] == ind)||(i < numclicks - 1 && inds[i+1] == ind)){
              ind = Math.floor(Math.random() * numlandmarks);
            }
            inds[i] = ind;
          }
        }
        uniqueinds = [...new Set(inds)];
        numuniqueinds = uniqueinds.length;
        console.log(uniqueinds);
        
      }
      // for (i = 0; i < numclicks; i++){
      //   // choose an ind based on numlandmarks
      //   ind = Math.floor(Math.random() * numlandmarks);
      //   while(ind == lastchosenind){
      //     ind = Math.floor(Math.random() * numlandmarks);
      //   }
      //   lastchosenind = ind;
      //   inds.push(ind);
      //   sdata.exploreforcedxlocs[coding.index][i] = sdata.blocklandmarkxlocs[coding.block][coding.trial][ind];
      //   sdata.exploreforcedylocs[coding.index][i] = sdata.blocklandmarkylocs[coding.block][coding.trial][ind];
      //   isGoal[i] = false;
      // }
      ...
      // while(numlandmarks != numuniqueinds){
      //   console.log('not all landmarks covered - regenerating');
      //   lastchosenind = 999;
      //   inds = [];
      //   for (i = 0; i < numclicks; i++){
      //     // choose an ind based on numlandmarks
    ...
    sdata.exploreforcedxlocs[coding.index][numclicks] = sdata.blockgoalxlocs[coding.block][coding.trial];
    sdata.exploreforcedylocs[coding.index][numclicks] = sdata.blockgoalylocs[coding.block][coding.trial];
    isGoal[numclicks] = true;
