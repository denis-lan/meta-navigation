...
function shuffle(array) {
  ...
  sdata.obstaclexlocs = new Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials);
  console.log(sdata.obstaclexlocs)
  sdata.obstacleylocs = new Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials);
  sdata.cond_obstaclexlocs = new Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials);
  sdata.cond_obstacleylocs = new Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials);
  for(i=0; i<1 + (parameters.nb_blocks-1) * parameters.nb_trials; i++){
    sdata.cond_obstaclexlocs[i] = '';
    sdata.cond_obstacleylocs[i] = '';
  }
  ...
  // sdata.cond_blockactionconditions = ['both'].concat(shuffle(['both', 'vonly', 'tonly', 'random']));
  sdata.cond_blockactionconditions = ['both', 'both', 'both', 'both', 'both'];
  sdata.cond_trialactionconditions = new Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials);
  for(i=0; i<1 + (parameters.nb_blocks-1) * parameters.nb_trials; i++){
    sdata.cond_trialactionconditions[i] = '';
  }
  // sdata.cond_freeorforced = [1].concat(Array(12).fill(1).concat(Array(12).fill(0)));
  sdata.cond_freeorforced = Array(1 + (parameters.nb_blocks-1) * parameters.nb_trials).fill(1);
  // sdata.cond_freeorforced = [0].concat(Array(24).fill(999));
  sdata.cond_blocknumlandmarks = shuffle([2, 4, 8, 16]);
  // sdata.cond_blocknumlandmarks = [2, 4, 8, 16, 2, 4, 8, 16]
  sdata.cond_numlandmarks = [3]
  for (i = 0; i < parameters.nb_blocks - 1; i++){
    // sdata.cond_numlandmarks = sdata.cond_numlandmarks.concat(shuffle([2, 4, 8, 16, 2, 4, 8, 16]))
    sdata.cond_numlandmarks = sdata.cond_numlandmarks.concat(new Array(parameters.nb_trials).fill(sdata.cond_blocknumlandmarks[i]));
  }
  
  sdata.cond_probetrials = [0].concat(Array((parameters.nb_blocks-1) * parameters.nb_trials).fill(0));
